...
Ember.Charts.TimeSeriesLabeler = Ember.Mixin.create({
  ...
  labelledSeconds: function (start, stop) {
    var seconds, skipVal;
    seconds = this.secondsBetween(start, stop);
    if (seconds.length > this.get('maxNumberOfLabels')) {
      skipVal = Math.ceil(seconds.length / this.get('maxNumberOfLabels'));
      return this.secondsBetween(start, stop, skipVal);
    } else {
      return seconds;
    }
  },
